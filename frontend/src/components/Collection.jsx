import { useState, useEffect, useRef } from 'react';
import { motion, useScroll, useTransform } from 'framer-motion';
import { useCart } from '../context/CartContext';
import { Link } from 'react-router-dom';

export default function Collection() {
  const [watches, setWatches] = useState([]);
  const targetRef = useRef(null);
  const { scrollYProgress } = useScroll({
    target: targetRef,
  });
  const { addToCart } = useCart();

  const x = useTransform(scrollYProgress, [0, 1], ["0%", "-50%"]);

  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const res = await fetch('/api/products');
        const data = await res.json();
        setWatches(data);
      } catch (error) {
        console.error('Failed to fetch products', error);
      }
    };
    fetchProducts();
  }, []);

  return (
    <section id="collections" ref={targetRef} className="relative h-[250vh] bg-luxury-black">
      <div className="sticky top-0 h-screen flex flex-col justify-center overflow-hidden">
        
        <div className="container mx-auto px-6 lg:px-12 mb-16">
          <h2 className="text-xl font-serif text-white/80 font-light tracking-wide">
            Curated <br /> <span className="text-white/40 italic">Collection.</span>
          </h2>
        </div>

        <motion.div style={{ x }} className="flex gap-24 pl-6 lg:pl-12 pb-20 w-[200vw]">
          {watches.map((watch) => (
            <div 
              key={watch._id} 
              className="group flex flex-col w-[300px] md:w-[450px] shrink-0"
            >
              {/* Stark Minimal Image Container */}
              <Link to={`/product/${watch._id}`}>
                <div className="w-full aspect-[4/5] bg-luxury-charcoal overflow-hidden relative mb-6">
                  <img 
                    src={watch.images && watch.images.length > 0 ? watch.images[0] : ''} 
                    alt={watch.name} 
                    className="w-full h-full object-cover object-center opacity-70 group-hover:opacity-100 transition-all duration-1000 group-hover:scale-105"
                  />
                </div>
              </Link>
              
              {/* Minimalist Typography */}
              <div className="flex justify-between items-start">
                <div>
                  <Link to={`/product/${watch._id}`}>
                    <h3 className="text-lg font-serif text-white mb-1 tracking-wide hover:text-white/70 transition-colors">
                      {watch.name}
                    </h3>
                  </Link>
                  <p className="text-white/40 text-[10px] uppercase tracking-[0.2em] w-[200px] truncate">
                    {watch.description}
                  </p>
                </div>
                <div className="flex flex-col items-end">
                  <p className="text-white/80 font-sans text-xs tracking-wider mb-2">
                    ${watch.price?.toLocaleString()}
                  </p>
                  <button 
                    onClick={(e) => {
                      e.preventDefault();
                      e.stopPropagation();
                      addToCart(watch);
                    }}
                    className="relative z-10 text-[9px] uppercase tracking-[0.2em] border-b border-transparent group-hover:border-white/30 text-white/40 group-hover:text-white transition-colors pb-1 cursor-pointer"
                  >
                    Add to Cart
                  </button>
                </div>
              </div>
            </div>
          ))}
        </motion.div>
      </div>
    </section>
  );
}
